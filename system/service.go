...
// Service handles all system-related API operations.
type Service struct {
	client *resty.Client
}
...
func (s *Service) parseErrorFromBody(body []byte, statusCode int) error {
	// Try to parse as nested error format (e.g., queue.role)
...
// parseNestedError parses errors with arbitrary nesting depth (e.g., queue.metadata.role).
